...
void preorder(node* r){
	if (r==NULL)return;
	if (r->parent !=NULL){
		cout << r->roll << " "<<r->name<< " size "<<r->Nsize<< " parent "<<r->parent->roll<<" is left "<<(r->parent->left == r)<<endl;
	}else 
		cout << r->roll << " "<<r->name<< " size "<<r->Nsize<< " parent NULL"<<endl;
	preorder(r->left);
	preorder(r->right);
}
class MAVLTree{
...
node* MAVLTree :: rightRotation(node *z)
{
    node* y = z->left;
    node* y_r = y->right;
    y->right = z;
    ...
    z->left = y_r;
    y->parent = NULL;
    if (y_r != NULL ) y_r->parent = z;
    z->Nheight = max(height(z->left), height(z->right))+1;
    y->Nheight = max(height(y->left), height(y->right))+1;
    y->Nsize = nodesize(y->left) + nodesize(y->right) + 1;
    z->Nsize = nodesize(z->left) + nodesize(z->right) + 1;
    return y;
}
node* MAVLTree :: leftRotation(node *z)
{
    node* y = z->right;
    node* y_l = y->left;
    y->left = z;
...
void MAVLTree:: insert(const int k , string s){
	...
	if (root_t1 == NULL ) {
		...
		if(b>1 || b<-1){
			...
			node* w = p->parent;
			if (p->roll > y->roll && y->roll > x->roll ){
				cout << "Inside T1 Undergoig righr rotation" << endl;
				if (w != NULL){
					if (w->left == p){
						w->left = rightRotation(p);
						w->left->parent = w;
					}
					else {
						w->right = rightRotation(p);
						w->right->parent = w;
					}
				}else {
					root_t2 = rightRotation(p);
				}
				//p->Nsize = nodesize(p->left) + nodesize(p->right) + 1;
				//y->Nsize = nodesize(y->left) + nodesize(y->right) + 1;
				//scout << "rotation comp R" << endl;
			}else if (p->roll > y->roll && x->roll > y->roll && x->roll < p->roll){
				cout << "Inside T1 Undergoig left rigt rotation" << endl;
				p->left = leftRotation(y);
				p->left->parent = p;
				//y->Nsize = nodesize(y->left) + nodesize(y->right) + 1;
				//x->Nsize = nodesize(x->left) + nodesize(x->right) + 1;
				//preorder(this->getroot_t1());cout << endl;
				if (w != NULL){
					if (w->left == p){
						w->left = rightRotation(p);
				...
				cout << "Inside T1 Undergoig left rotation" << endl;
				if (w != NULL){
					if (w->left == p){
						w->left = leftRotation(p);
						w->left->parent = w;
					}
					else {
						w->right = leftRotation(p);
						w->right->parent = w;
					}
				}else {
					root_t2 = leftRotation(p);
				}
				...
				cout << "Inside T1 Undergoig right left rotation" << endl;
				p->right = rightRotation(y);
				p->right->parent=p;
				//y->Nsize = nodesize(y->left) + nodesize(y->right) + 1;
				//x->Nsize = nodesize(x->left) + nodesize(x->right) + 1;
				//preorder(this->getroot_t1());cout << endl;
...
void MAVLTree :: deleteNode(const int k){
	...
	/*}else{
		cout << "is root_t1 deleting "<< k <<endl;
		if (curr->left == NULL ||p,x curr->right == NULL){
			...
			if (temp == NULL){
				...
				delete curr;
				cout << "have one child , deleted "<< k<<endl;
			}
		}else {
			node* suc = this->minValueNode(curr->right);
			int c_roll = curr->roll;
			curr->roll = suc->roll;
			suc->roll = c_roll;
			this->remove(c_roll);
			Tsize--;
			cout << "have two child , deleted "<< k<<endl;
		}
	}*/
...
/*node* MAVLTree :: deleteNode(node* curr, int k){
    if (curr == NULL){
        return curr;
    }
    if ( k < curr->roll ){
        ...
        if((curr->left == NULL) || (curr->right == NULL)){
        	cout << "inside T1  deleting "<<curr->roll<<endl;
            node* t;
	...
	//cout << "3 "<<endl;
    curr->Nheight = max(height(curr->left),height(curr->right)) + 1;
 	//cout << "height of "<<curr->roll<< " is " <<  curr->Nheight<<endl;
 	//cout << "height of left  "<<curr->roll<< " is " <<  height(curr->left)<<endl;
 	//cout << "is balanced "<<isBalnode(curr)<<endl;
 	int b = isBalnode(curr);
 	cout << "curr "<<curr->roll << " is "<<b<<endl;
    if ( b>1 && isBalnode(curr->left)>=0){
    	cout << "Undergoig righr rotation" << endl;
    	rightRotation(curr, curr->left,root_t1);
    	return curr->parent;
    }
    else if (b>1 && isBalnode(curr->left)<=0)
    {
        cout << "Undergoig left righr rotation" << endl;
        leftRotation(curr->left, curr->left->right,root_t1);
        //cout << "curr left is " << curr->left->roll<<endl;
       	rightRotation(curr,curr->left,root_t1);
       	//preorder(this->getroot_t1());cout << endl;
       	return curr->parent;
    ...
    else if (b<-1 && isBalnode(curr->right)>=0){
        cout << "Undergoig left rotation" << endl;
        leftRotation(curr, curr->right,root_t1);
        /// cout << "parent od curr is "<< curr->parent->roll<<endl;
        return curr->parent;
    }
       ...
       cout << "Undergoig righr left rotation" << endl;
       rightRotation(curr->right, curr->right->left,root_t1);
       leftRotation(curr,curr->right,root_t1);
       return curr->parent;
    }
    //cout << "Returning curr "<<curr->roll<< endl;
    return curr;
}*/
...
void MAVLTree :: deleteNode1(node* curr){
    ...
    if ( b>1 && isBalnode(curr->left)>=0){
    	cout << "Undergoig righr rotation" << endl;
    	rightRotation(curr, curr->left);
    	return curr->parent;
    }
    else if (b>1 && isBalnode(curr->left)<=0)
    {
        cout << "Undergoig left righr rotation" << endl;
        leftRotation(curr->left, curr->left->right);
        //cout << "curr left is " << curr->left->roll<<endl;
       	rightRotation(curr,curr->left);
       	//preorder(this->getroot_t1());cout << endl;
       	return curr->parent;
       ...
       cout << "Undergoig righr left rotation" << endl;
       rightRotation(curr->right, curr->right->left);
       leftRotation(curr,curr->right);
       return curr->parent;
    }
    //cout << "Returning curr "<<curr->roll<< endl;
...
int MAVLTree :: get_place(int k){
	node* temp = root_t1;
	while(temp != NULL && temp->roll != k){
		if (temp->roll < k) temp = temp->right;
		else temp = temp->left;
